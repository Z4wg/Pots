...
3. Open [`lib/supabase.ts`](lib/supabase.ts) and replace `SUPABASE_URL` and
   `SUPABASE_ANON_KEY` with your project's values (Project Settings → API).
4. Restart `npx expo start`. The app auto-detects the keys and switches from the mock
   backend to live Supabase realtime. The header shows **LIVE POT** instead of
   **DEMO POT · LOCAL**.
...
Open Banking / Revolut merchant + MCC codes; here the seed and dev buttons set it
...
app/
  ...
  onboarding.tsx         5-card archetype quiz + animated reveal
...
- Supabase keys live in `lib/supabase.ts`; any LLM key lives in the Edge Function.
